...
# NOTE: KNOWLEDGE_FILE (legacy) and KNOWLEDGE_DB are set in Configuration below.
...
# Extract learnings from structured agent output (writes to SQLite)
extract_learnings() {
  local story_id="$1"
...
EOF
  ...
  # === SQLite Knowledge Store Statistics ===
  if [ -f "$KNOWLEDGE_DB" ]; then
    cat >> "$report_file" <<EOF
...
# Run a review agent and parse its verdict
# Args: $1=story_id, $2=agent_name, $3=log_prefix, $4=event_prefix, $5=prompt, $6=timeout,
#       $7+=extra claude flags (optional)
# Outputs: REVIEW_VERDICT, REVIEW_ISSUE_COUNT (global)
# Returns: 0=agent ran OK, 1=agent failed (approve by default)
run_review_agent() {
  local story_id="$1" agent_name="$2" log_prefix="$3" event_prefix="$4"
  local prompt="$5" review_timeout="$6"
  shift 6
  local extra_flags=("$@")

  log "${log_prefix}-$story_id" "Starting $agent_name review..."
  emit_event "${event_prefix}.start" "{}" "$story_id"

  local review_output
  review_output=$($TIMEOUT_CMD $review_timeout env -u CLAUDECODE claude -p "$prompt" \
    --agent "$agent_name" \
    --dangerously-skip-permissions \
  ...
  if [ $review_exit -ne 0 ]; then
    log "${log_prefix}-$story_id" "$agent_name failed to run (exit $review_exit) — approving by default"
    emit_event "${event_prefix}.complete" "{\"verdict\":\"approve\",\"error\":true}" "$story_id"
    REVIEW_VERDICT="approve"
    REVIEW_ISSUE_COUNT=0
    return 1
  fi

  REVIEW_VERDICT=$(echo "$review_output" | jq -r '
    .verdict // .result.verdict //
    (.. | objects | .verdict // empty) // "approve"
  ' 2>/dev/null | head -1)
  [ -z "$REVIEW_VERDICT" ] && REVIEW_VERDICT="approve"

  REVIEW_ISSUE_COUNT=$(echo "$review_output" | jq '
    [.. | objects | .issues // [] | .[]] | length
  ' 2>/dev/null || echo "0")

  log "${log_prefix}-$story_id" "Verdict: $REVIEW_VERDICT ($REVIEW_ISSUE_COUNT issues)"
  emit_event "${event_prefix}.complete" "{\"verdict\":\"$REVIEW_VERDICT\",\"issues\":$REVIEW_ISSUE_COUNT}" "$story_id"
  return 0
}

# Spec compliance review (Stage 1, mandatory)
# Args: story_id
...
run_spec_review() {
  ...
  if [ "$SPEC_REVIEW_ENABLED" != "true" ]; then return 0; fi
...
Output your structured JSON verdict as described in your system prompt."
  ...
  [ "$spec_timeout" -lt 60 ] && spec_timeout=60

  REVIEW_VERDICT="" REVIEW_ISSUE_COUNT=0
  run_review_agent "$story_id" "spec-reviewer" "SPEC" "spec_review" "$spec_prompt" "$spec_timeout"

  case "$REVIEW_VERDICT" in
    approve) return 0 ;;
    reject)  log "SPEC-$story_id" "Spec compliance rejected ($REVIEW_ISSUE_COUNT issues)"; return 1 ;;
    *)       return 0 ;;
  esac
}
...
run_code_review() {
  ...
  if [ "$CODE_REVIEW_ENABLED" != "true" ]; then return 0; fi
...
Output your review as the structured JSON verdict described in your system prompt."
  ...
  REVIEW_VERDICT="" REVIEW_ISSUE_COUNT=0
  run_review_agent "$story_id" "code-reviewer" "REVIEW" "review" "$review_prompt" "$review_timeout" \
    --output-format json --no-session-persistence

  case "$REVIEW_VERDICT" in
    approve)          return 0 ;;
    request_changes)  log "REVIEW-$story_id" "Code review: changes requested ($REVIEW_ISSUE_COUNT issues)"; return 1 ;;
    reject)           log "REVIEW-$story_id" "Code review: rejected (spec compliance failed)"; return 2 ;;
    *)                return 0 ;;
  esac
}

# ============================================================================
# Parallel Mode — Wave-Based Execution
# ============================================================================
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  CONTEXT_BRIEF_FILE=$(mktemp)
  {
    echo "# Story: $CURRENT_STORY"
    echo '```json'
    jq --arg id "$CURRENT_STORY" '.userStories[] | select(.id == $id)' "$PRD_FILE" 2>/dev/null
    echo '```'
  } > "$CONTEXT_BRIEF_FILE"
  ...
  # Build the full prompt: story context + agent instructions
...
Please address the issue and try again."
      ...
      # Build retry context brief (knowledge injection handled by SubagentStart hook)
      RETRY_BRIEF_FILE=$(mktemp)
      {
        echo "# Story: $CURRENT_STORY (retry)"
        echo '```json'
        jq --arg id "$CURRENT_STORY" '.userStories[] | select(.id == $id)' "$PRD_FILE" 2>/dev/null
        echo '```'
        echo ""
        echo "## Previous Failure Context"
        echo "$RETRY_CONTEXT"
      } > "$RETRY_BRIEF_FILE"
  ...
  # Extract learnings from structured agent output
