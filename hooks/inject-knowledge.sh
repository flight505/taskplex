...
# 1b. Dependency stories (full details)
DEP_STORY_IDS=$(jq -r --arg id "$STORY_ID" \
  '.userStories[] | select(.id == $id) | .depends_on // [] | .[]' \
  "$PRD_FILE" 2>/dev/null)

if [ -n "$DEP_STORY_IDS" ]; then
  CONTEXT="${CONTEXT}## Dependency Stories (full)
"
  while IFS= read -r dep_sid; do
    [ -z "$dep_sid" ] && continue
    DEP_STORY_JSON=$(jq --arg id "$dep_sid" '.userStories[] | select(.id == $id)' "$PRD_FILE" 2>/dev/null)
    if [ -n "$DEP_STORY_JSON" ]; then
...
# 1c. All other stories (id + title + status summary only)
OTHER_STORIES=$(jq -r --arg id "$STORY_ID" \
  '[.userStories[] | select(.id != $id and (.depends_on // [] | index($id) | not)) | {id, title, status: (if .passes then "completed" elif .status == "skipped" then "skipped" else .status // "pending" end)}] | map("\(.id): \(.title) [\(.status)]") | .[]' \
  "$PRD_FILE" 2>/dev/null)

if [ -n "$OTHER_STORIES" ]; then
  CONTEXT="${CONTEXT}## Other Stories (summary)
${OTHER_STORIES}
...
# 3b. Established patterns (promoted from repeated learnings — no decay)
PATTERNS=$(query_patterns "$KNOWLEDGE_DB" 5 2>/dev/null)
if [ -n "$PATTERNS" ]; then
  CONTEXT="${CONTEXT}## Established Patterns (high confidence, cross-session)
"
  while IFS='|' read -r p_content p_category p_count p_confidence; do
    [ -z "$p_content" ] && continue
    CONTEXT="${CONTEXT}- [${p_category}] (seen ${p_count}x) ${p_content}
"
  done <<< "$PATTERNS"
  CONTEXT="${CONTEXT}
"
